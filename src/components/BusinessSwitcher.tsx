...
export default function BusinessSwitcher() {
  ...
  const { selectedBusiness, setSelectedBusiness, refreshBusinessData } = useBusiness();
  const { user } = useAuth();
  ...
  const handleSelectBusiness = async (business: Business) => {
    ...
    if (['/dashboard', '/items', '/invoices', '/purchases', '/suppliers', '/customers'].includes(currentPath)) {
      ...
      <DropdownMenu>
          ...
          >
            <Avatar className="h-6 w-6">
              <AvatarFallback className="text-xs bg-primary/20 text-primary">
                {selectedBusiness ? getBusinessInitials(selectedBusiness.name) : <Building2 className="h-3 w-3" />}
              </AvatarFallback>
            </Avatar>
        ...
        <DropdownMenuContent align="end" className="w-[280px] bg-card">
          ...
          {loading ? (
            ...
            <div className="p-4 text-sm text-center text-muted-foreground">No businesses yet</div>
          ) : (
